...
export default function App() {
  ...
  const sumCat = (id) =>
    (data[id] || []).reduce((s, i) => s + Number(i.amount || 0), 0);
    ...
    <div className="rail">
      <div className="rail-top">
        {items.map((item) => (
          <div
            key={item.id}
            className={`rail-icon ${currentTab === item.id ? 'active' : ''} ${
              longPressId === item.id ? 'wiggle' : ''
            } ${
              dragOver === item.id && dragging !== item.id ? 'drag-over' : ''
            }`}
      ...
      <div className="rail-bottom" onClick={onOpenSettings} title="設定">
        ⚙
      </div>
    </div>
  );
      ...
      <div className="user-info">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="user-email">
            <span className="sync-dot"></span>Firebase 已連線
          </div>
        </div>
      </div>
...
function Content({ settings, store, currentTab, data, sumCat }) {
  ...
  if (currentTab === 'trends')
    return <TrendsView settings={settings} store={store} />;
    ...
    return (
      <div className="content-area">
        ...
        <div className="page-sub">
          All Your Revenue And Expenses ・ {store} ・{' '}
          <span className="sync-dot"></span>即時同步
        </div>
        <div className="cards">
          <div className="card revenue">
            <div className="label">總營業額</div>
            <div className="value">{fmt(rev)}</div>
          </div>
          <div className="card expense">
            <div className="label">總支出</div>
            <div className="value">{fmt(totalExp)}</div>
          </div>
          <div className="card profit">
            <div className="label">
              淨利 <span className="badge">{margin}%</span>
            </div>
            <div className="value">{fmt(profit)}</div>
          </div>
        </div>
  ...
  if (!cat)
    return (
      <div className="content-area">
        <div className="page-title">找不到此頁</div>
      </div>
    );
...
function CompareView({ settings }) {
  ...
  const getVendorBreakdown = (catId) => {
    ...
    settings.stores.forEach((s) => {
      const items = (allData[s]?.[catId] || []).filter((i) =>
  ...
  const storeTotals = settings.stores.map((s) => ({
    store: s,
    amount: sumFor(s, selectedCat),
  }));
  ...
  return (
    <div className="content-area">
      ...
      <div className="chart-card">
        <h4>
          {cat?.icon} {cat?.name} ・ 各店合計（本
          {PERIODS.find((p) => p.id === period)?.name}）
        </h4>
        <div style={{ overflowX: 'auto' }}>
          <div
            className="bar-chart"
            style={{ minWidth: `${settings.stores.length * 90}px` }}
          >
            {storeTotals.map((s, i) => (
      ...
      <div className="vendor-card">
        ...
        {vendorList.length === 0 ? (
          <div
            className="empty"
            style={{ padding: 30, textAlign: 'center', color: '#aaa' }}
          >
            此分類尚無資料
          </div>
        ) : (
            ...
            <table>
              ...
              <tbody>
                {vendorList.map(([name, byStore]) => {
                  const total = Object.values(byStore).reduce(
                    (s, v) => s + v,
                    0
                  );
                  return (
...
function TrendsView({ settings, store }) {
  ...
  const [loading, setLoading] = useState(true);
  const [selectedCat, setSelectedCat] = useState(
    settings.categories[0]?.id || 'revenue'
  );
  const [period, setPeriod] = useState('month');
    ...
    if (period === 'day') {
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        buckets.push({
          ...
          label: `${y.toString().slice(2)}Q${q + 1}`,
        });
      }
    } else if (period === 'year') {
      for (let i = 4; i >= 0; i--) {
  ...
  const areaPath =
    series.values.length > 0
      ? `M ${PAD_L},${PAD_T + chartH} L ${linePoints
          .split(' ')
          .join(' L ')} L ${PAD_L + (n - 1) * xStep},${PAD_T + chartH} Z`
      : '';
  ...
  return (
    <div className="content-area">
      ...
      <div className="page-sub">
        {store} 的 {cat?.name} 時間變化趨勢（要看其他門店請從左上切換）
      </div>
      ...
      <div className="cards">
        ...
        <div className="card">
          ...
          <div
            className="value"
            style={{ fontSize: 26, color: isUp ? '#11998e' : '#eb3349' }}
          >
            {isUp ? '+' : ''}
            {fmt(change)}
          </div>
      ...
      <div className="chart-card">
        <h4>
          {cat?.icon} {cat?.name} 趨勢 ・{' '}
          {PERIODS.find((p) => p.id === period)?.name}線
        </h4>
          ...
          <table>
            ...
            <tbody>
              {series.labels.map((label, i) => {
                ...
                const diff = v - prev;
                const diffPct =
                  prev > 0 ? ((diff / prev) * 100).toFixed(1) : null;
                return (
                  <tr key={i}>
                    <td>
                      <b>{label}</b>
...
function CategoryView({ store, cat, items }) {
  ...
  return (
    <div className="content-area">
      ...
      <div className="page-sub">
        {cat.type === 'revenue' ? '記錄每日營業收入' : '記錄此項支出明細'} ・{' '}
        {store}
      </div>
              ...
              items.map((it) => (
                <tr key={it.id}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <button className="del-btn" onClick={() => del(it.id)}>
                      刪除
                    </button>
                  </td>
                </tr>
...
function SettingsModal({ settings, onSave, onClose }) {
  ...
  const addStore = () =>
    setDraft({ ...draft, stores: [...draft.stores, '新門店'] });
  const delStore = (i) => {
    if (
      !confirm(
  ...
  const delCat = (i) => {
    if (
      !confirm(
        `確定刪除分類「${draft.categories[i].name}」？\n⚠ 該分類已輸入的資料仍會保留在 Firebase。`
      )
    )
      return;
    setDraft({
          ...
          <div className="setting-group">
            ...
            {draft.stores.map((s, i) => (
              <div key={i} className="list-row">
                ...
                />
                <button className="del-btn" onClick={() => delStore(i)}>
                  刪除
                </button>
              </div>
            ))}
            <button className="add-btn" onClick={addStore}>
              + 新增門店
            </button>
          </div>
          ...
          <div className="setting-group">
            ...
            {draft.categories.map((c, i) => (
              <div key={i} className="cat-row">
                ...
                <select
                  value={c.type}
                  onChange={(e) => updateCat(i, 'type', e.target.value)}
                >
                  <option value="revenue">收入</option>
                  <option value="expense">支出</option>
                </select>
                <button className="del-btn" onClick={() => delCat(i)}>
                  刪除
                </button>
              </div>
            ))}
